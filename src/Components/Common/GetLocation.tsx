...
//   console.log('🚀 ~ GetLocation ~ savedLocation:', savedLocation);
...
const GetLocation = () => {
  ...
  const dispatch = useAppDispatch();
  const {coordinates} = useAppSelector(state => state.places);
  const navigation = useAppNavigation();
        ...
        <KeyboardAvoidingView
          ...
          <View>
            ...
            <DebouncedSearchInput
              label="Search city, area or neighbourhood"
              value={query}
              onChange={setQuery}
              onSelect={(item: any) => getCoordinates(item.place_id, item)}
